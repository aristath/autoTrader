...
// setupReconciliationService creates a reconciliation service with mock cash manager for testing
func setupReconciliationService(t *testing.T, db *sql.DB) (*ReconciliationService, *BalanceService, *MockCashManager, *BalanceRepository) {
	log := zerolog.New(nil).Level(zerolog.Disabled)
	balanceRepo := NewBalanceRepository(db, log)
	bucketRepo := NewBucketRepository(db, log)
	cashManager := NewMockCashManager()
	balanceService := NewBalanceService(cashManager, balanceRepo, bucketRepo, log)
	reconciliationService := NewReconciliationService(balanceService, balanceRepo, bucketRepo, log)
	return reconciliationService, balanceService, cashManager, balanceRepo
}
...
func TestCheckInvariant_Matched(t *testing.T) {
	...
	service, _, cashManager, _ := setupReconciliationService(t, db)

	// Set virtual balance to 1000 EUR using cash manager
...
func TestCheckInvariant_ZeroBalance(t *testing.T) {
	...
	service, _, _, _ := setupReconciliationService(t, db)
...
func TestReconcile_AlreadyReconciled(t *testing.T) {
	...
	service, balanceService, cashManager, _ := setupReconciliationService(t, db)
	...
	// Verify no changes (reuse same service instance to access same cashManager)
	balance, err := balanceService.GetBalanceAmount("core", "EUR")
	assert.NoError(t, err)
...
func TestReconcile_SmallDifference_AutoCorrect(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 1002.0)
	assert.NoError(t, err)
	...
	// Verify core balance was corrected (reuse same service instance)
	balance, err := balanceService.GetBalanceAmount("core", "EUR")
	assert.NoError(t, err)
	assert.Equal(t, 1000.0, balance)

	// Verify transaction recorded (need balanceRepo for this)
	_, _, _, balanceRepo := setupReconciliationService(t, db)
	txs, err := balanceRepo.GetRecentTransactions("core", 1)
	assert.NoError(t, err)
	assert.Len(t, txs, 1)
	assert.Equal(t, TransactionTypeReallocation, txs[0].Type)
	assert.Equal(t, 2.0, txs[0].Amount) // Transaction amounts are now stored as absolute values
}
...
func TestReconcile_SmallNegativeDifference_AutoCorrect(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 998.0)
	assert.NoError(t, err)
...
func TestReconcile_LargeDifference_NoAutoCorrect(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 1100.0)
	assert.NoError(t, err)
...
func TestReconcile_CustomThreshold(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 1010.0)
	assert.NoError(t, err)
...
func TestReconcile_ExactlyAtThreshold(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 1005.0)
	assert.NoError(t, err)
...
func TestReconcileAll_MultipleCurrencies(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 1002.0)
	assert.NoError(t, err)
	_, err = cashManager.AdjustCashBalance("core", "USD", 502.0)
	assert.NoError(t, err)
	...
	// Verify balances corrected (reuse same service instance)
	eurBalance, _ := balanceService.GetBalanceAmount("core", "EUR")
	assert.Equal(t, 1000.0, eurBalance)

	usdBalance, _ := balanceService.GetBalanceAmount("core", "USD")
...
func TestReconcileAll_MixedResults(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 1002.0)
	assert.NoError(t, err)
	_, err = cashManager.AdjustCashBalance("core", "USD", 600.0)
	assert.NoError(t, err)
...
func TestInitializeFromBrokerage_FirstTime(t *testing.T) {
	...
	service, balanceService, _, balanceRepo := setupReconciliationService(t, db)
	...
	// Verify core balances initialized (reuse same service instance)
	eurBalance, err := balanceService.GetBalanceAmount("core", "EUR")
	assert.NoError(t, err)
	assert.Equal(t, 5000.0, eurBalance)

	usdBalance, err := balanceService.GetBalanceAmount("core", "USD")
	assert.NoError(t, err)
...
func TestInitializeFromBrokerage_ZeroBalance(t *testing.T) {
	...
	service, balanceService, _, _ := setupReconciliationService(t, db)
...
func TestForceReconcileToCore_AdjustDown(t *testing.T) {
	...
	_, err := cashManager.AdjustCashBalance("core", "EUR", 1200.0)
	assert.NoError(t, err)
...
func TestForceReconcileToCore_WithSatellites(t *testing.T) {
	...
	assert.NoError(t, err)
	_, err = cashManager.AdjustCashBalance("sat1", "EUR", 300.0)
	assert.NoError(t, err)
	...
	coreBalance, err := balanceService.GetBalanceAmount("core", "EUR")
	assert.NoError(t, err)
	assert.Equal(t, 700.0, coreBalance)

	// Satellite should be unchanged
	sat1Balance, err := balanceService.GetBalanceAmount("sat1", "EUR")
	assert.NoError(t, err)
	assert.Equal(t, 300.0, sat1Balance)

	// Total should match actual
	total, err := balanceService.GetTotalByCurrency("EUR")
	assert.NoError(t, err)
...
func TestDiagnoseDiscrepancy_WithRecentTransactions(t *testing.T) {
	...
	service, _, cashManager, balanceRepo := setupReconciliationService(t, db)
