...
def test_explicit_downgrade_requires_timestamp():
    ...
    assert is_explicit_downgrade({"ai_research_multiplier": 0.5, "ai_research_multiplier_updated_at": None}) is False
    assert is_explicit_downgrade({"ai_research_multiplier": 0.2}) is False


def test_explicit_downgrade_true_when_rated_at_or_below_neutral():
    assert (
        is_explicit_downgrade(
            {"ai_research_multiplier": 0.5, "ai_research_multiplier_updated_at": "2026-06-01T00:00:00Z"}
        )
        is True
    )
    assert (
        is_explicit_downgrade(
            {"ai_research_multiplier": 0.1, "ai_research_multiplier_updated_at": "2026-06-01T00:00:00Z"}
        )
        is True
    )
...
def test_explicit_downgrade_false_above_neutral():
    ...
    assert (
        is_explicit_downgrade(
            {"ai_research_multiplier": 0.8, "ai_research_multiplier_updated_at": "2026-06-01T00:00:00Z"}
        )
        is False
    )
...
def test_decay_moves_above_neutral_value_toward_neutral():
    ...
    assert decayed_ai_research_multiplier(0.7, 0.9) == pytest.approx(0.68, abs=1e-9)
...
def test_decay_moves_below_neutral_value_toward_neutral():
    ...
    assert decayed_ai_research_multiplier(0.3, 0.9) == pytest.approx(0.32, abs=1e-9)


def test_decay_leaves_neutral_value_unchanged():
    assert decayed_ai_research_multiplier(0.5, 0.9) == 0.5


def test_decay_with_factor_one_is_identity():
    # No fade applied.
    assert decayed_ai_research_multiplier(0.7, 1.0) == 0.7


def test_decay_with_factor_zero_snaps_to_neutral():
    assert decayed_ai_research_multiplier(0.7, 0.0) == 0.5
    assert decayed_ai_research_multiplier(0.0, 0.0) == 0.5
...
def test_decay_52_iterations_converges_close_to_neutral():
    ...
    value = 1.0
    for _ in range(52):
        value = decayed_ai_research_multiplier(value, DEFAULT_DECAY_FADE_FACTOR)
...
def test_decay_normalizes_non_finite_inputs():
    ...
    # `normalize_ai_research_multiplier`), which means a single decay step leaves
    # them at neutral — there's no "true" extremity to fade from.
    assert decayed_ai_research_multiplier(float("nan"), 0.9) == 0.5
    assert decayed_ai_research_multiplier(float("inf"), 0.9) == 0.5
    assert decayed_ai_research_multiplier(float("-inf"), 0.9) == 0.5
...
def test_decay_clamps_factor_into_unit_interval():
    ...
    assert decayed_ai_research_multiplier(0.7, 1.5) == decayed_ai_research_multiplier(0.7, 1.0)
    assert decayed_ai_research_multiplier(0.7, -0.1) == decayed_ai_research_multiplier(0.7, 0.0)


def test_decay_handles_extreme_values():
    # 0.0 (avoid) → 0.05 above the floor after one step.
    assert decayed_ai_research_multiplier(0.0, 0.9) == pytest.approx(0.05, abs=1e-9)
    assert decayed_ai_research_multiplier(1.0, 0.9) == pytest.approx(0.95, abs=1e-9)
...
def test_non_finite_preference_values_fall_back_to_neutral():
    assert normalize_ai_research_multiplier(float("nan")) == 0.5
    assert normalize_ai_research_multiplier(float("inf")) == 0.5
