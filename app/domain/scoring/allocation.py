"""
Allocation Fit Score - Portfolio awareness for smart rebalancing.
...
from app.domain.scoring.constants import (
    ALLOCATION_WEIGHT_GEOGRAPHY,
    ALLOCATION_WEIGHT_INDUSTRY,
    ALLOCATION_WEIGHT_AVERAGING_DOWN,
...
def calculate_allocation_fit_score(
    ...
    Returns:
        AllocationFitScore with all components
    """
    ...
    if position_value > 0:
        ...
        # Cost basis bonus: If price < avg purchase price, boost priority
        if (portfolio_context.position_avg_prices and
            ...
            if avg_price and current_price and avg_price > 0:
                ...
                if price_vs_avg < 0:
                    # We're underwater - buying more will lower our average
                    loss_pct = abs(price_vs_avg)
                    if loss_pct <= COST_BASIS_BOOST_THRESHOLD:
                        # Linear boost: 5% loss = +0.1, 20% loss = +0.4
        ...
        if total_value > 0:
            ...
            if position_pct > CONCENTRATION_HIGH:  # >10%
                averaging_down_score *= 0.7
            elif position_pct > CONCENTRATION_MED:  # 5-10%
                averaging_down_score *= 0.9
    else:
        # Don't own this stock - neutral
        averaging_down_score = 0.5

    # Combined score
    total = (
        geo_gap_score * ALLOCATION_WEIGHT_GEOGRAPHY +
        industry_gap_score * ALLOCATION_WEIGHT_INDUSTRY +
        averaging_down_score * ALLOCATION_WEIGHT_AVERAGING_DOWN
    )
