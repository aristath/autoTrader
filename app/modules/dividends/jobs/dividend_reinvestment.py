...
# Go API base URL for dividend operations (migrated to Go)
...
async def auto_reinvest_dividends() -> None:
    ...
    # HTTP client for Go API calls (dividend operations migrated to Go)
    async with httpx.AsyncClient(timeout=30.0) as http_client:
        try:
            ...
            # Get all unreinvested dividends from Go API (migrated to Go)
            response = await http_client.get(
                f"{GO_API_BASE_URL}/api/dividends/unreinvested",
                params={"min_amount_eur": 0.0},
            )
            response.raise_for_status()
            dividend_data = response.json()
            dividends = [DividendRecord(**d) for d in dividend_data]
            ...
            for symbol, symbol_dividends in grouped_dividends.items():
                ...
                if total_amount < min_trade_size:
                    ...
                    # Set pending bonus for each small dividend via Go API
                    for dividend in symbol_dividends:
                        if dividend.id is not None:
                            response = await http_client.post(
                                f"{GO_API_BASE_URL}/api/dividends/{dividend.id}/pending-bonus",
                                json={"amount": dividend.amount_eur},
                            )
                            response.raise_for_status()
                    continue
                    ...
                    for symbol in low_yield_dividends.keys():
                        if symbol in grouped_dividends:
                            dividends_to_mark[symbol] = [
                                d.id
                                for d in grouped_dividends[symbol]
                                if d.id is not None
                            ]
                else:
                    # No opportunities found, set pending bonuses via Go API
                    logger.info(
            ...
            if recommendations:
                ...
                try:
                    ...
                    # Mark dividends as reinvested for successful trades via Go API
                        ...
                        if result.get("status") == "success":
                            ...
                            if symbol in dividends_to_mark:
                                ...
                                for dividend_id in dividends_to_mark[symbol]:
                                    ...
                                    response = await http_client.post(
                                        f"{GO_API_BASE_URL}/api/dividends/{dividend_id}/mark-reinvested",
                                        json={"quantity": int(executed_quantity)},
                                    )
                                    response.raise_for_status()
                                logger.info(
                    ...
                    logger.error(
                        f"Error executing dividend reinvestment trades: {e}",
                        exc_info=True,
                    )
            else:
