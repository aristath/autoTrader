...
| `decay:ai_research_multipliers` | Daily walk over `securities`: any row whose slider is ≥ 7 days old gets one step closer to neutral via `value = 0.5 + (value − 0.5) × 0.9`. Touching the slider resets the timer. |
